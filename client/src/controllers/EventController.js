...
import * as FormData from '../utils//formData.js';
...
export class EventController {
  ...
  async registerEvent(request) {
    return BackendService.handleRequest(request, async (req) => {
      const formData = await FormData.parseFormData(req);
      ...
      if (thumbnail && thumbnail.size > 0) {
        thumbnailURL = await Storage.uploadFileToR2(this.env, thumbnail, `events/thumbnails/${title}-${Date.now()}`);
      }
      const banner = formData.get('banner');
      if (banner && banner.size > 0) {
        bannerURL = await Storage.uploadFileToR2(this.env, banner, `events/banners/${title}-${Date.now()}`);
      }
