  ...
  async registerOrganization(request) {
    return BackendService.handleRequest(request, async (req) => {
      ...
      if (thumbnail && thumbnail.size > 0) {
        ...
        thumbnailURL = await Storage.uploadFileToR2(this.env, thumbnail, `thumbnails/Thumb_${cleanName}`);
      }
      const banner = formData.get('banner');
      if (banner && banner.size > 0) {
        const cleanName = name.replace(/\s+/g, '_');
        bannerURL = await Storage.uploadFileToR2(this.env, banner, `banners/Banner_${cleanName}`);
      }
